...
export function ForYouSection({
  ...
  const discoverQuery = useForYouEvents({
    enabled: showFollowFeedSheet && activeTab === 'discover',
  });
  ...
  const fullDiscoverEvents = discoverQuery.data || discoverEvents;
  ...
  const sheetEvents = activeTab === 'discover' ? fullDiscoverEvents : events;
  const groupedSheetEvents = groupEventsByDate(sheetEvents);
    ...
    <div className='flex flex-col'>
      ...
      >
        <div className='flex flex-col gap-4 px-4 pb-8'>
          {activeTab === 'discover' && discoverQuery.isLoading && !discoverQuery.data ? (
            <div className='flex flex-col gap-2'>
              {['sheet-ske-a', 'sheet-ske-b', 'sheet-ske-c', 'sheet-ske-d'].map((skeletonKey) => (
                <div
                  key={skeletonKey}
                  className='flex items-start gap-4 rounded-3xl border border-gray-200 bg-gray-50 p-4'
                >
          ...
          ) : activeTab === 'discover' && discoverQuery.error && !discoverQuery.data ? (
            <div className='rounded-2xl border border-amber-200 bg-amber-50 p-4 text-amber-900'>
              <div className='mb-2 flex items-center gap-2 text-sm font-semibold'>
                <AlertTriangle className='h-4 w-4' />
                Could not load discover events
              </div>
              <p className='text-sm text-amber-800'>{discoverQuery.error.message}</p>
            </div>
          ) : groupedSheetEvents.length === 0 ? (
            <div className='flex flex-col items-center justify-center py-12 text-center'>
              <div className='mb-4 rounded-2xl bg-gray-100 p-4'>
                <Calendar className='h-8 w-9 text-gray-400' />
              </div>
