          ...
          {showUnitMenu && (
            <div className='absolute right-0 top-full z-10 mt-1 min-w-[120px] rounded-lg border border-gray-200 bg-white py-1 shadow-lg'>
              ...
              >
                ...
                {displayUnit === 'F' && <div className='h-2 w-2 rounded-full bg-red-600'></div>}
              </button>
              <button
                onClick={() => {
                  setDisplayUnit('C');
              ...
              >
                <span>Celsius</span>
                {displayUnit === 'C' && <div className='h-2 w-2 rounded-full bg-red-600'></div>}
              </button>
            </div>
          )}
        </div>
    ...
    >
      ...
      <div className='mt-6 rounded-lg bg-blue-50 p-3'>
        <div className='text-xs text-blue-800'>
          <strong>Note:</strong> This shows current weather conditions at the event location, not
          the actual weather from the event date.
        </div>
      </div>
    </MasterScrollableSheet>
  );
