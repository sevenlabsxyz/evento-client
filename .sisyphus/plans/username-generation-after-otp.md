...
- [x] 1. Add client-side username generation + availability helper
  ...
  **Agent-Executed QA Scenarios**:
  ...
  Scenario: Generate valid username from name+email with collisions
    ...
    Steps:
      1. Execute a script that calls the helper with `email="alex@example.com"`, `name="Alex"` and a mocked availability function that returns unavailable for `alex`, available for `alex2`.
      2. Assert: returned username equals `alex2`.
      3. Assert: username is lowercase and alphanumeric.
  ...
  Scenario: Reserved word is avoided
    ...
    Steps:
      1. Execute a script with input that yields `"api"` as a base.
      2. Assert: returned username is not `api` and uses numeric suffix or alternate combo.
...
- [x] 2. Wire username generation into post-OTP verification flow
  ...
  **Agent-Executed QA Scenarios**:
  ...
  Scenario: Post-OTP flow triggers username creation
    ...
    Steps:
      1. Trigger OTP verification (via existing dev flow or mocked auth response).
      2. Observe network: PATCH `/v1/user` with `username` and `name`.
      3. Call `GET /v1/user` to confirm stored username.
  ...
  Scenario: Collision handling appends numeric suffix
    ...
    Steps:
      1. Trigger OTP verification for new user.
      2. Verify helper attempts `alex`, sees unavailable, then uses `alex2`.
      3. Confirm `PATCH /v1/user` payload uses `alex2`.
  ...
  Scenario: Failure does not block login
    ...
    Steps:
      1. Trigger OTP verification.
      2. Confirm user session is established even if username update fails.
