...
import { debugLog, debugError, debugApiResponse } from '@/lib/utils/debug';
    ...
    queryFn: async (): Promise<Event> => {
      ...
      try {
        ...
        if (!response || typeof response !== 'object') {
          debugError('useEventDetails', 'Invalid response format', 
        ...
        if (!transformed) {
          debugError('useEventDetails', 'Failed to transform API response', 
        ...
        debugError('useEventDetails', 'Failed to fetch event details', error, { eventId });
        throw error;
      }
    },
    enabled: !!eventId,
