...
export function transformApiEventToDisplay(
  ...
  const endDateTime = formatEventDate(
    apiEvent.computed_end_date,
    apiEvent.timezone,
  );
  ...
  if (apiEvent.user_details) {
    const creatorHost: EventHost = {
      ...
      name:
        apiEvent.user_details.name ||
        apiEvent.user_details.username ||
        "Unknown",
  ...
  return {
    ...
    location: location,
    coverImages: apiEvent.cover
      ? [getOptimizedCoverUrl(apiEvent.cover, "detail")]
      : [],
    galleryImages: galleryItems.map((item) =>
      getOptimizedCoverUrl(item.url, "detail"),
    ),
    ...
    details: {
      profileUrl:
        apiEvent.contrib_cashapp || apiEvent.contrib_venmo || undefined,
