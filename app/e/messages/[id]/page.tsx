...
import { ArrowLeft, MoreHorizontal, Send, Plus, ImageIcon, Paperclip, FileText } from "lucide-react"
...
export default function SingleChatPage() {
  ...
  const messages = [
    ...
    },
    {
      id: 4,
      sender: "You",
      content: "The ramen place I mentioned is just 5 minutes from your hotel. Perfect for jet lag recovery! 🍜",
      time: "10:35 AM",
      isMe: true,
    },
    {
      ...
      <div className="flex items-center gap-3 flex-1">
        <div className="relative">
          <img
            ...
            className={`object-cover ${isGroup ? "w-10 h-10 rounded-lg" : "w-10 h-10 rounded-full"}`}
          />
        ...
        <div className="flex-1 min-w-0">
          <h3 className="font-bold text-black text-lg truncate">{currentChat.name}</h3>
    ...
    <div className="max-w-sm mx-auto bg-white min-h-screen flex flex-col">
            ...
            <div key={msg.id} className={`flex ${msg.isMe ? "justify-end" : "justify-start"}`}>
              <div className={`flex gap-2 max-w-[80%] ${msg.isMe ? "flex-row-reverse" : "flex-row"}`}>
                ...
                <div className={`flex flex-col ${msg.isMe ? "items-end" : "items-start"}`}>
                  {!msg.isMe && isGroup && <span className="text-xs text-gray-500 mb-1 px-3">{msg.sender}</span>}
                  <div
                    className={`px-4 py-2 rounded-2xl ${
                      msg.isMe ? "bg-orange-500 text-white rounded-br-md" : "bg-gray-100 text-gray-900 rounded-bl-md"
                    }`}
                  >
      ...
      <div className="fixed bottom-0 left-1/2 transform -translate-x-1/2 w-full max-w-sm bg-white border-t border-gray-100 p-4">
        <div className="flex items-center gap-3">
          <ReusableDropdown
            trigger={
              <Button variant="ghost" size="icon" className="h-8 w-8 flex-shrink-0 rounded-full bg-gray-100">
