...
describe('profile-events utilities', () => {
  ...
  it('ignores impossible display date parts instead of normalizing them into another day', () => {
    const invalidDisplayDate = createEvent('invalid-display-date', '2026-04-30T18:00:00.000Z', {
      start_date_year: 2026,
      start_date_month: 4,
      start_date_day: 31,
      start_date_hours: 10,
    ...
    expect(getProfileEventDateKey(invalidDisplayDate)).toBe('2026-04-30');
    expect(sortAndGroupProfileEvents([invalidDisplayDate], 'asc')).toEqual([
      {
        date: '2026-04-30',
        events: [invalidDisplayDate],
      },
    ]);
  });
});
